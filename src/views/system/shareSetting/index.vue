<template>
  <div class="page">
    <ElForm class="query-form" inline :label-width="80" :model="formData">
      <ElFormItem label="等级名称 :" prop="contract_address" style="width: 25%">
      ...
      <div class="action-groups">
        ...
        <ElButton class="button" plain type="primary" @click="editRow($event)">
          新增
        </ElButton>
      </div>
    </ElForm>
    ...
    >
      ...
      <ElTableColumn label="二级佣金率(%)" prop="rate2" />
      <ElTableColumn label="三级佣金率(%)" prop="rate3" />
...
<script setup>
  ...
  async function getData() {
    ...
    const res = await getLevelDividedList(params)
    loading.value = false
  ...
  async function deleteRow(row) {
    const { msg } = await deleteLevelDivided({ id: row.id })
  ...
  const onSwitchChange = async (row) => {
    loading.value = true
    await switchLevel({ id: row.id, field: `status` })
    loading.value = false
  }
</script>
<style>
